# -*- encoding: utf-8 -*-
# Created on 2016-02-10 19:13:34
# Project: cnki
...
class Handler(BaseHandler):
    base_url = "http://dict.cnki.net/dict_result.aspx?searchword="
    mydb = Mysql()
    crawl_config = {
    }
    ...
    def detail_page(self, response):
        ...
        for item in response.doc('.text11Green').items():
            eng = item.parent().prev('tr')
            chinese = item.text()
            english = eng.text()
            data = {
                'word': word,
    ...
    def on_result(self, result):
        self.mydb.insert_data('cnki', result)
